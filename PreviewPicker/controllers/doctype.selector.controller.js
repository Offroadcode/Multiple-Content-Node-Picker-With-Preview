angular.module("umbraco").controller("doctype.selector.controller", function($scope, docTypeSelectorResource) {
	...
	*/
	$scope.buildDocTypeOptionList = function() {
		docTypeSelectorResource.getAll().then(function(docTypes) {
			if (docTypes && docTypes.length) {
				docTypes.forEach(function(docType) {
					...
					if ($scope.model.value == option.alias) {
	...
	*/
	$scope.setVariables = function() {
		$scope.docTypes = [{alias: '', id: 0, name: "Select DocType"}];
	...
	*/
	$scope.updateValue = function() {
		...
		if ($scope.selectedDocType.id != 0) {
			updatedValue = $scope.selectedDocType.alias;
		}
